...
async def process_image_task(number, image_id, user_weight):
    try:
        ...
        # Pre-filter
        check = gemini.generate_content(["Is this a fuel price board? Yes/No", {"mime_type": "image/jpeg", "data": img_bytes}])
        if "yes" not in check.text.lower():
            send_whatsapp(number, "❌ Geen prijzenbord herkend.")
            return
